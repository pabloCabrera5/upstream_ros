...
#include "ros/ros.h"
#include <iostream>
//get the action client APIs which are used to implement action clients
...
int main (int argc, char **argv)
{
  ...
  // create an instance of a goal and send a goal to the action
  ...
  //wait for the action from the server to return
  bool finished_before_timeout = ac.waitForResult(ros::Duration(atoi(argv[2])));
  //Preempting task
  ac.cancelGoal(); // if it is not finished , it will preempt the action
